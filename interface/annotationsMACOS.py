...
class AnnotatorFrame(LabelFrame):
	...
	def __init__(self, parent, **kwargs):
		...
		self._canvas.grid(row = 0, rowspan = nbrRows, column = nbrScalePerLine, columnspan = nbrCols - nbrScalePerLine + 1, sticky = "WE")
		
		canWidth, canHeight = (float(self._canvas['width']), float(self._canvas['height']))

		#Upper Left
		self._quarters[(-1,1)] = self._canvas.create_arc(self.__canH(canWidth, 1.3 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 1.0),\
								self.__canH(canWidth, 7.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 7.0), start = 90, extent = 90)

		#Left
		self._quarters[(-1,0)] = self._canvas.create_rectangle(self.__canH(canWidth, 1.3 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 4.0),\
									self.__canH(canWidth, 4.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 5.0))

		#Loer Left
		self._quarters[(-1,-1)] = self._canvas.create_arc(self.__canH(canWidth, 1.3 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 2.0),\
								self.__canH(canWidth, 7.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 8.0), start = 180, extent = 90)



		#Upper
		self._quarters[(0,1)] = self._canvas.create_rectangle(self.__canH(canWidth, 4.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 1.0),\
									self.__canH(canWidth, 5.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 4.0))

		#Center
		self._quarters[(0,0)] = self._canvas.create_rectangle(self.__canH(canWidth, 4.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 4.0),\
									self.__canH(canWidth, 5.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 5.0))

		#Lower
		self._quarters[(0,-1)] = self._canvas.create_rectangle(self.__canH(canWidth, 4.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 5.0),\
									self.__canH(canWidth, 5.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 8.0))



		#Upper Right
		self._quarters[(1,1)] = self._canvas.create_arc(self.__canH(canWidth, 2.15 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 1.0),\
								self.__canH(canWidth, 7.85 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 7.0), start = 0, extent = 90)

		#Right
		self._quarters[(1,0)] = self._canvas.create_rectangle(self.__canH(canWidth, 5.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 4.0),\
									self.__canH(canWidth, 7.85 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 5.0))

		#Lower Right
		self._quarters[(1,-1)] = self._canvas.create_arc(self.__canH(canWidth, 2.15 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 2.0),\
								self.__canH(canWidth, 7.85 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 8.0), start = 270, extent = 90)
		...
		self._canLabels = {'Joy' : self._canvas.create_text(self.__canH(canWidth, 8.25 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 3.0), text = 'Joy'),\
				...
				'Positive': self._canvas.create_text(self.__canH(canWidth, 8.4 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 4.5), font = ('Purisa', 8), text = 'Positive'),\
				'Frustrated': self._canvas.create_text(self.__canH(canWidth, 1.0 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 2.0), font = ('Purisa', 8), text = 'Frustrated'),\
				'Anxiety' : self._canvas.create_text(self.__canH(canWidth, 0.65 + MACOS_GRAPHIC_MARGIN), self.__canH(canHeight, 5.0), font = ('Purisa', 8), text = 'Anxiety')}
	...
	"""
	Correct the bug for resolution of the scale for tkinter
	"""
